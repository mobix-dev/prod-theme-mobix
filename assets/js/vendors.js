(function ($) {
	...
	*/
	VecCarousel = function (arguments) {
		var self = this;
		...
		this.waitImagesLoaded = arguments["waitImagesLoaded"] || false;
	...
	VecCarousel.prototype.loadPictures = function () {
		...
		if (imagesCarousel.length > 0 && self.waitImagesLoaded) {
			imagesCarousel.each(function () {
				this.addEventListener("load", function () {
					...
					if (loadedImagesCount == imagesCarousel.length) {
						...
						$(self.vecCarousel).on('widthChanged', function() {
							self.setVecCarousel();
						});
					}
				});
	...
	VecCarousel.prototype.setVecCarousel = function () {
		...
		$(self.vecCarousel).find("div[data-vec-carousel-screen]").each(function () {
			if ($(this).height() > maxHeightCarouselScreen) {
				maxHeightCarouselScreen = $(this).innerHeight();
            }
		});
...
(function ($) {
	...
	var codeSequence = [38, 38, 40, 40, 37, 39, 37, 39, 66, 65];
	var bufferSequence = [];

	function initIt() {
		$("body").on("keyup", function (event) {
            bufferSequence.push(event.keyCode);

            if (bufferSequence.length > codeSequence.length) {
                bufferSequence.shift();
            }

            if (codeSequence.length === bufferSequence.length) {
                var isCodeSame = codeSequence.every(function (element, index) {
                    return element === bufferSequence[index];
                });

                if (isCodeSame) {
					if ($('#lil-pete').length === 0) {
						$('body').prepend('<div id="lil-pete-container"><video id="lil-pete" autoplay><source src="/wp-content/uploads/2021/06/lil_pete.mp4" type="video/mp4" /></video></div>');
						setTimeout(function () {
							$('#lil-pete-container').addClass('is-opened');
						}, 100);

						$('#lil-pete').on('ended', function () {
							$('#lil-pete-container').removeClass('is-opened');

							setTimeout(function () {
								$('#lil-pete-container').remove();
							}, getDurationTransition('#lil-pete-container'));
						});

						$('#lil-pete').on('click', function () {
							if ($('#lil-pete')[0].paused) {
								$('#lil-pete')[0].play();
							}
						});
					}
